    ...
    def compute_loss(self, embeddings, labels, indices_tuple=None):
        ...
        for inst_idx in range(M):
            ...
            if torch.sum(one_mask) > 1:
                ...
                embeddings_concat = torch.cat((one_queries, one_centroids))
                labels_concat = torch.cat((one_labels, one_labels))
                ...
                # make only centroid vectors be positive/negative references (select every other)
                indices_tuple = [x.view(len(one_labels), -1) for x in indices_tuple]
                indices_tuple = [x.chunk(2, dim=1)[1].flatten() for x in indices_tuple]

...
class ProxyAnchorLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        pos_term = lmu.logsumexp(-self.alpha * (cos - self.margin) * miner_weights, keep_mask=pos_mask, add_one=True, dim=0)
        neg_term = lmu.logsumexp(self.alpha * (cos + self.margin) * miner_weights, keep_mask=neg_mask, add_one=True, dim=0)

...
class BatchHardMiner(BaseTupleMiner):
    def __init__(self, **kwargs):
        ...
        self.add_to_recordable_attributes(
            list_of_names=[
                "hardest_triplet_dist",
                "hardest_pos_pair_dist",
                "hardest_neg_pair_dist",
            ],
            is_stat=True,
        )
    ...
    def mine(self, embeddings, labels, ref_emb, ref_labels):
        ...
        pos_func = (
            self.get_min_per_row if self.distance.is_inverted else self.get_max_per_row
        )
        neg_func = (
            self.get_max_per_row if self.distance.is_inverted else self.get_min_per_row
        )

        (hardest_positive_dist, hardest_positive_indices), a1p_keep = pos_func(
            mat, a1_idx, p_idx
        )
        (hardest_negative_dist, hardest_negative_indices), a2n_keep = neg_func(
            mat, a2_idx, n_idx
        )
        a_keep_idx = torch.where(a1p_keep & a2n_keep)
        self.set_stats(
            hardest_positive_dist[a_keep_idx], hardest_negative_dist[a_keep_idx]
        )
        ...
        if self.collect_stats:
            with torch.no_grad():
                ...
                if len_pd > 0 and len_pn > 0:
                    self.hardest_triplet_dist = pos_func(

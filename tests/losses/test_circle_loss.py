        ...
        for dtype in TEST_DTYPES:
            ...
            for i in range(len(embeddings)):
                ...
                if len(pos_logits) == 0 or len(neg_logits) == 0:
                    ...
                    pos_logits = torch.cat(pos_logits, dim=0) if len(pos_logits) > 1 else pos_logits[0]
                    neg_logits = torch.cat(neg_logits, dim=0) if len(neg_logits) > 1 else neg_logits[0]
                    correct_total += torch.nn.functional.softplus(torch.logsumexp(pos_logits, dim=0) + torch.logsumexp(neg_logits, dim=0))

        ...
        for dtype in TEST_DTYPES:
            ...
            for a1, p in pos_pairs:
                ...
                for a2, n in neg_pairs:
                    ...
                    if a2 == a1:
                        neg_pair_component += torch.exp(
                            neg_margin - torch.sqrt(torch.sum((anchor - negative) ** 2))
                        )
                    elif a2 == p:
                        neg_pair_component += torch.exp(
                            neg_margin
                            - torch.sqrt(torch.sum((positive - negative) ** 2))
                        )
                    else:
                        continue
                total_loss += (
                    torch.relu(torch.log(neg_pair_component) + pos_pair_component) ** 2
                )

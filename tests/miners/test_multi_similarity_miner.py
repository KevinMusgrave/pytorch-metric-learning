        ...
        for dtype in TEST_DTYPES:
            for distance in [CosineSimilarity(), LpDistance()]:
                ...
                for a1,p,ap_sim in pos_pairs:
                    most_difficult = c_f.neg_inf(dtype) if distance.is_inverted else c_f.pos_inf(dtype)
                    for a2,n,an_sim in neg_pairs:
                        if a2==a1:
                            condition = (an_sim > most_difficult) if distance.is_inverted else (an_sim < most_difficult)
                            if condition:
                                most_difficult = an_sim
                    condition = (ap_sim < most_difficult + epsilon) if distance.is_inverted else (ap_sim > most_difficult - epsilon)
                    if condition:
                        correct_a1.append(a1)
                        correct_p.append(p)

                for a2,n,an_sim in neg_pairs:
                    most_difficult = c_f.pos_inf(dtype) if distance.is_inverted else c_f.neg_inf(dtype)
                    for a1,p,ap_sim in pos_pairs:
                        if a2==a1:
                            condition = (ap_sim < most_difficult) if distance.is_inverted else (ap_sim > most_difficult)
                            if condition:
                                most_difficult = ap_sim
                    condition = (an_sim > most_difficult - epsilon) if distance.is_inverted else (an_sim < most_difficult + epsilon)
                    if condition:

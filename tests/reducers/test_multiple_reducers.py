...
class TestMultipleReducers(unittest.TestCase):
    def test_multiple_reducers(self):
        ...
        lossesA = torch.randn(batch_size)
        lossesB = torch.randn(batch_size)
        ...
        for indices, reduction_type in [(torch.arange(batch_size), "element"),
            ...
            loss_dict = {"lossA": {"losses": lossesA, "indices": indices, "reduction_type": reduction_type},
                        "lossB": {"losses": lossesB, "indices": indices, "reduction_type": reduction_type, "divisor_summands": {"partA": 32, "partB": 15}}}
            output = reducer(loss_dict, embeddings, labels)
            correct_output = (torch.mean(lossesA[lossesA>0])) + (torch.sum(lossesB) / (32+15))

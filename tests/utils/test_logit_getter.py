...
from pytorch_metric_learning.losses import ArcFaceLoss, NormalizedSoftmaxLoss, ProxyAnchorLoss
...
class TestLogitGetter(unittest.TestCase):
    ...
    def test_logit_getter(self):
        ...
        for dtype in TEST_DTYPES:
            embeddings = torch.randn(batch_size, embedding_size).to(self.device).type(dtype)
            ...
            self.helper_tester(loss1, embeddings, batch_size, num_classes, layer_name="W")

            #test specifying a distance metric
            self.helper_tester(loss1, embeddings, batch_size, num_classes, distance=LpDistance())
            ...
            self.helper_tester(loss1, embeddings, batch_size, num_classes, transpose=True)

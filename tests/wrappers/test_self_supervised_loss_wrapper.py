...
class TestSelfSupervisedLossWrapper(unittest.TestCase):
    def test_ssl_wrapper_same(self):
        ...
        for dtype in TEST_DTYPES:
            ...
            embeddings = torch.randn(300, 1024, requires_grad=True, dtype=dtype,).to(TEST_DEVICE) 
            embeddings = torch.nn.functional.normalize(embeddings)
            ref_emb = embeddings
            zero = torch.tensor(0., dtype=dtype).to(TEST_DEVICE)

            atol = 1e-2
            for loss_fn in loss_fns:
                loss = loss_fn(embeddings, ref_emb)
                self.assertTrue(
                    torch.isclose(zero, loss, atol=atol)
                )
    
    def test_ssl_wrapper_all(self):
        for dtype in TEST_DTYPES:
            embeddings = torch.randn(100, 256, requires_grad=True, dtype=dtype,).to(TEST_DEVICE) 
            embeddings = torch.nn.functional.normalize(embeddings)

            ref_emb = torch.randn(100, 256, requires_grad=True, dtype=dtype,).to(TEST_DEVICE) 
